...
export const updateListing = async (req, res, next) => {
  const listing = await Listing.findById(req.params.id);
  if (!listing) {
    return next(errorHandler(404, "Error,Listing not found"));
  }
  if (req.user.id !== listing.userRef) {
    return next(errorHandler(401, "Unauthorized to update other listing"));
  }
...
export const getListings = async (req, res, next) => {
  try {
    ...
    if (offer === undefined || offer === "false") {
      // $in means search inside database
      offer = { $in: [false, true] };
    }
    ...
    const listings = await Listing.find({
      // options - i means not caring of uppercase or lowercase , search term... means any term
